...
const Message = ( props ) => { // ({ message, authorId })
  ...
  let { message, authorId } = props;  // destrukturyzacja teraz, zamiast w atrybutach funkcji - potzrebny dostęp do "props"
  ...
  return (
    <article className="message" >
      <section>
        ...
        <div className={ message.authorId === authorId ? "message-container my-message" : "message-container" } >
          <p className="author">
            <strong>{ message.authorId }</strong> o&nbsp;
            <strong className="time">{ convertTimestampToHMSString( message.timestamp ) }</strong>
          </p>
          <p className={ message.authorId === authorId ? "my-post-text" : "" } >
            { message.text }
          </p>
        </div>
      </section>
